package com.javarush.test.level03.lesson03.task03;

/* 10 процентов
Реализуйте метод addTenPercent, который увеличивает переданное целое число на 10%.
Исправьте ошибку в сигнатуре метода.
Подсказка: подумайте, какие у этого метода входные и выходные данные.
*/
public class Solution
{
    public static  double addTenPercent(int i) {//заменили void  на  double
        //i = i + 1 / 10; если написать так то мы вернули бы целое значение
        double j = i + i * 0.1;
//в double  метод мы можем передавать целое но выводить нужно double.
        return j;//напишите тут ваш код
    }

    public static void main(String[] args)
    {
        System.out.println(addTenPercent(12));
    }
}
